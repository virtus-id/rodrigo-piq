"""`GAB-NFI-06` — investimento bloqueado (`T-129`).
...
Classificação de Ativos, congelado em 2026-09-09). A §14.16 é normativa e
literal, mesma régua dos `GAB-AI`.

    `GAB-NFI-06`
      Cenário .......... `DISPOSICAO_USO_INVESTIMENTO=SIM`,
                         `LIQUIDEZ_INVESTIMENTOS=BLOQUEADO`
      Resultado ........ `NAO_MOBILIZAR`

Âncora: `AC-88` (`US-19`), `RF-53`, §14.3.1 Regra 1, §14.16.
...
def test_gab_nfi_06_investimento_bloqueado() -> None:
    """`GAB-NFI-06` · `AC-88` · §14.16: `DISPOSICAO_USO_INVESTIMENTO=SIM`,
    `LIQUIDEZ_INVESTIMENTOS=BLOQUEADO` → `NAO_MOBILIZAR`.

    A Regra 1 (bloqueio, §14.3.1) vence independentemente da disposição ser
    `SIM` — a ordem de precedência estrita das seis regras nunca deixa a
    Regra 4 (que também se aplicaria por outros campos) alcançar este
    cenário.
    """
    ...
    assertar_exato(obtido, CLASSIFICACAO_MOBILIZACAO.NAO_MOBILIZAR)
